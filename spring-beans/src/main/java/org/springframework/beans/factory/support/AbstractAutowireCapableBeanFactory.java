...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	 ...
	 * 使用给定的父类创建一个新的AbstractAutowireCapableBeanFactory。
	 *
	...
	public Object applyBeanPostProcessorsBeforeInitialization(Object existingBean, String beanName)
		...
		// 循环遍历已经定义的后置处理器（默认有 ApplicationContextAwareProcessor、BeanPostProcessorChecker、ApplicationListenerDetector），对bean进行加工处理，并返回
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// mbdToUse表示bean定义的实体RootBeanDefinition实例
		RootBeanDefinition mbdToUse = mbd;

		// 获得mbd的beanClass属性，例：<bean class="org.springframework.Persion">中的class属性
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		synchronized (mbd.postProcessingLock) {
			// postProcessed默认为false，作用是标识是否已经执行过实现 MergedBeanDefinitionPostProcessor 的后置处理器
			if (!mbd.postProcessed) {
				...
				// 标识已经执行过MergedBeanDefinitionPostProcessor后置处理器
		...
		try {
			...
			/** Demo不涉及 */
			/*
			1.执行实现了BeanPostProcessor的后置处理器（默认无处理）
			2.执行 initMethods 方法,即<bean init-method=""/>
			3.如果bean实现了BeanFactoryAware、BeanClassLoaderAware、BeanNameAware，有逻辑
			*/
		...
		/** Demo不涉及-start */
		try {
			// 只有定义了DestroyMethod方法，即<bean destroy-method=""/> 或者 实现 DestructionAwareBeanPostProcessor 的bean，才有效
	...
	/**
	 * 执行beanPostProcessors集合中的MergedBeanDefinitionPostProcessor#postProcessMergedBeanDefinition方法（默认集合只包含ApplicationListenerDetector实例）
	 *
	 * <p>
	...
	protected void applyMergedBeanDefinitionPostProcessors(RootBeanDefinition mbd, Class<?> beanType, String beanName) {
		// 循环遍历 beanPostProcessors 集合中的 MergedBeanDefinitionPostProcessor 实例
		for (BeanPostProcessor bp : getBeanPostProcessors()) {
			// 默认只包含 ApplicationListenerDetector 实例属于 MergedBeanDefinitionPostProcessor
			if (bp instanceof MergedBeanDefinitionPostProcessor) {
				MergedBeanDefinitionPostProcessor bdp = (MergedBeanDefinitionPostProcessor) bp;
				// 以{beanName->是否是单例(boolean)}的形式存储到singletonNames
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			// bean为null，beforeInstantiationResolved为false
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// 通过<bean class="工厂类" factory-method="工厂类执行的方法"/>实例化bean
	...
	protected Object getObjectForBeanInstance(
		...
		// 调用从 AbstractBeanFactory 的getObjectForBeanInstance
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// hasInstAwareBpps 表示是否定义了实现 InstantiationAwareBeanPostProcessor 的bean
		boolean hasInstAwareBpps = hasInstantiationAwareBeanPostProcessors();
		// needsDepCheck 表示是否配置<dependency-check=""/>
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		// original 表示bean下<property>标签的集合
	...
	protected Object initializeBean(final String beanName, final Object bean, @Nullable RootBeanDefinition mbd) {
		...
		if (System.getSecurityManager() != null) {
			...
			// 只有用户定义的bean实现了BeanFactoryAware、BeanClassLoaderAware、BeanNameAware，才有效
			invokeAwareMethods(beanName, bean);
		}
		Object wrappedBean = bean;
		// isSynthetic 默认为false，与AOP切面相关
		if (mbd == null || !mbd.isSynthetic()) {
			// 遍历并调用后置处理器(即实现 BeanPostProcessor 的类)，执行 postProcessBeforeInitialization 方法，用于执行init-metho方法前，bean的加工
			wrappedBean = applyBeanPostProcessorsBeforeInitialization(wrappedBean, beanName);
		}
		try {
			// 执行 initMethods 方法,即<bean init-method=""/>
		...
		if (mbd == null || !mbd.isSynthetic()) {
			// 遍历并调用后置处理器(即实现 BeanPostProcessor 的类)，执行 postProcessAfterInitialization 方法，用于执行init-metho方法后，bean的加工
